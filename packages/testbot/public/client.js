...
var Hivekit = {
    ...
    connect: function (user) {
        ...
        if (user && user.id) {
            Hivekit.setCookie('hivekit_guid', user.id, 1);
    ...
    connectWebhook: function () {
        var that = this;
        if (Hivekit.getCookie('hivekit_guid')) {
            that.guid = Hivekit.getCookie('hivekit_guid');
            connectEvent = 'welcome_back';
        } else {
            that.guid = that.generate_guid();
            Hivekit.setCookie('hivekit_guid', that.guid, 1);
        }
    ...
    receiveCommand: function (event) {
        switch (event.data.name) {
            case 'trigger':
                // tell Hivekit to trigger a specific script/thread
                console.log('TRIGGER', event.data.script, event.data.thread);
                Hivekit.triggerScript(event.data.script, event.data.thread);
                break;
            case 'identify':
                // link this account info to this user
                console.log('IDENTIFY', event.data.user);
                Hivekit.identifyUser(event.data.user);
                break;
            case 'connect':
                // link this account info to this user
                Hivekit.connect(event.data.user);
                break;
            default:
