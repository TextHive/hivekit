...
import { HivekitBotFrameworkAdapter } from './adapter';
import { BotWorker } from './botworker';
import { HivekitConversationState } from './conversationState';
...
export interface HivekitConfiguration {
     ...
     * The adapter is responsible for translating platform-specific messages into the format understood by Hivekit and BotBuilder.
     */
    adapter?: any;
    ...
    /**
     * Disable webserver. If true, Hivekit will not create a webserver or expose any webhook endpoints automatically. Defaults to false.
     * For an example of how to use your own Express, [see this sample code](https://github.com/texthive/hivekit/blob/main/packages/testbot/custom_express.js).
     */
 ...
 */
export interface HivekitMessage {
    /**
 ...
 * ```
 */
export interface HivekitHandler {
    (bot: BotWorker, message: HivekitMessage): Promise<any>;
}

/**
 * Defines a trigger, including the type, pattern and handler function to fire if triggered.
 */
interface HivekitTrigger {
    /**
     * string, regexp or function
     */
    type: string;
    pattern: string | RegExp | { (message: HivekitMessage): Promise<boolean> };
    handler: HivekitHandler;
}

/**
 * An interface for plugins that can contain multiple middlewares as well as an init function.
 */
export interface HivekitPlugin {
    name: string;
    middlewares?: {
        [key: string]: any[];
    };
    init?: (hivekit: Hivekit) => void;
...
export class Hivekit {
     ...
     */
    private _config: HivekitConfiguration;

    /**
     * _events contains the list of all events for which Hivekit has registered handlers.
     * Each key in this object points to an array of handler functions bound to that event.
     */
    private _events: {
        [key: string]: HivekitHandler[];
    } = {};

    /**
     * _triggers contains a list of trigger patterns htat Hivekit will watch for.
     * Each key in this object points to an array of patterns and their associated handlers.
     * Each key represents an event type.
     */
    private _triggers: {
        [key: string]: HivekitTrigger[];
    } = {};

    /**
     * _interrupts contains a list of trigger patterns htat Hivekit will watch for and fire BEFORE firing any normal triggers.
     * Each key in this object points to an array of patterns and their associated handlers.
     * Each key represents an event type.
     */
    private _interrupts: {
     ...
     */
    private conversationState: HivekitConversationState;

    /**
     * _deps contains a list of all dependencies that Hivekit must load before being ready to operate.
     * see addDep(), completeDep() and ready()
     */
    private _deps: {};

    /**
     * contains an array of functions that will fire when Hivekit has completely booted.
     */
    private _bootCompleteHandlers: { (): void }[];

    /**
     * The current version of Hivekit Core
     */
    public version: string = require('../package.json').version;

    /**
     * Middleware endpoints available for plugins and features to extend Hivekit.
     * Endpoints available are: spawn, ingest, receive, send.
     *
     * To bind a middleware function to Hivekit:
     * ```javascript
    ...
    /**
     * A BotBuilder DialogSet that serves as the top level dialog container for the Hivekit app
     */
    public dialogSet: DialogSet;

    /**
     * The path of the main Hivekit SDK, used to generate relative paths
     */
    public PATH: string;

    /**
     * Indicates whether or not Hivekit has fully booted.
     */
     ...
     * ```
     *
     * @param config Configuration for this instance of Hivekit
     */
    public constructor(config: HivekitConfiguration) {
        // Set the path where Hivekit's core lib is found.
        this.PATH = __dirname;
        ...
        debug('Booting Hivekit ', this.version);
        ...
        if (!this._config.storage) {
            ...
            if (this._config.disable_console !== true) {
                ...
                console.warn('** To preserve dialog state, specify a storage adapter in your Hivekit config:');
                console.warn('** const controller = new Hivekit({storage: myStorageAdapter});');
            }
        } else {
            this.storage = this._config.storage;
        }

        this.conversationState = new HivekitConversationState(this.storage);
        ...
        if (!this._config.adapter) {
            ...
            this.adapter = new HivekitBotFrameworkAdapter(adapterConfig);
            if (this.webserver) {
                if (this._config.disable_console !== true) {
        ...
        if (this.adapter) {
            // MAGIC: Treat the adapter as a hivekit plugin
     ...
     * @param plugin_or_function A plugin module in the form of function(hivekit) {...} that returns {name, middlewares, init} or an object in the same form.
     */
    public usePlugin(plugin_or_function: ((hivekit: Hivekit) => HivekitPlugin) | HivekitPlugin): void {
        let plugin: HivekitPlugin;
     ...
     */
    private registerPlugin(name: string, endpoints: HivekitPlugin): void {
        if (this._config.disable_console !== true) {
     ...
     * ```
     *
     * @param handler {function} A function to run when Hivekit is booted and ready to run.
     */
    ...
    private configureWebhookEndpoint(): void {
        if (this.webserver) {
            this.webserver.post(this._config.webhook_uri, (req, res) => {
                ...
                // which we can then use to turn into a HivekitMessage
    ...
    public async handleTurn(turnContext: TurnContext): Promise<any> {
        ...
        // Turn this turnContext into a Hivekit message.
        const message: HivekitMessage = {
            ...
            // if Hivekit has further classified this message, use that sub-type rather than the Activity type
            type: (turnContext.activity.channelData && turnContext.activity.channelData.hivekitEventType) ? turnContext.activity.channelData.hivekitEventType : turnContext.activity.type,
        ...
        // Stash the Hivekit message in
        turnContext.turnState.set('hivekitMessage', message);
     ...
     * @param message {HivekitMessage} an incoming message
     */
    private async processTriggersAndEvents(bot: BotWorker, message: HivekitMessage): Promise<any> {
     ...
     * @param message {HivekitMessage} an incoming message
     */
    private async listenForTriggers(bot: BotWorker, message: HivekitMessage): Promise<any> {
     ...
     * @param message {HivekitMessage} an incoming message
     */
    private async listenForInterrupts(bot: BotWorker, message: HivekitMessage): Promise<any> {
     ...
     * @param trigger {HivekitTrigger} a trigger definition
     * @param message {HivekitMessage} an incoming message
     */
    private async testTrigger(trigger: HivekitTrigger, message: HivekitMessage): Promise<boolean> {
     ...
     * @param handler {HivekitHandler}  a function that will be called should the pattern be matched
     */
    public hears(patterns: (string | RegExp | { (message: HivekitMessage): Promise<boolean> })[] | RegExp | string | { (message: HivekitMessage): Promise<boolean> }, events: string | string[], handler: HivekitHandler): void {
     ...
     * @param handler {HivekitHandler}  a function that will be called should the pattern be matched
     */
    public interrupts(patterns: (string | RegExp | { (message: HivekitMessage): Promise<boolean> })[] | RegExp | RegExp[] | string | { (message: HivekitMessage): Promise<boolean> }, events: string | string[], handler: HivekitHandler): void {
        if (!Array.isArray(patterns)) {
            patterns = [patterns];
        }
     ...
     * @param handler {HivekitHandler} a handler function that will fire whenever one of the named events is received.
     */
    public on(events: string | string[], handler: HivekitHandler): void {
        if (typeof events === 'string') {
            events = events.split(/,/).map(e => e.trim());
        }
     ...
     * @param bot {BotWorker} a BotWorker instance created using `controller.spawn()`
     * @param message {HivekitMessagE} An incoming message or event
     */
    public async trigger(event: string, bot?: BotWorker, message?: HivekitMessage): Promise<any> {
    ...
    public async spawn(config?: any, custom_adapter?: BotAdapter): Promise<BotWorker> {
        ...
        if (adapter.hivekit_worker) {
            const CustomBotWorker = adapter.hivekit_worker;
    ...
    public addDialog(dialog: Dialog): void {
        ...
        this.dialogSet.add(new WaterfallDialog(dialog.id + ':hivekit-wrapper', [
     ...
     */
    public afterDialog(dialog: Dialog | string, handler: HivekitHandler): void {
        let id = '';
