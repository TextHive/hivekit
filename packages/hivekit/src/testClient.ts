...
import { Hivekit } from './core';

/**
 * A client for testing dialogs in isolation.
 */
export class HivekitTestClient {
     ...
     * @param bot (Required) The Hivekit bot that has the skill to test.
     ...
     */
    public constructor(channelId: string, bot: Hivekit, dialogToTest: string | string[], initialDialogOptions?: any, middlewares?: Middleware[], conversationState?: ConversationState)
    public constructor(testAdapter: TestAdapter, bot: Hivekit, dialogToTest: string | string[], initialDialogOptions?: any, middlewares?: Middleware[], conversationState?: ConversationState)
    public constructor(channelOrAdapter: string | TestAdapter, bot: Hivekit, dialogToTest: string | string[], initialDialogOptions?: any, middlewares?: Middleware[], conversationState?: ConversationState) {
        ...
        if (Array.isArray(dialogToTest)) {
            dialogToTest.forEach((dialogName) => {
                ...
                targetDialogs.push(
                    bot.dialogSet.find(dialogName + ':hivekit-wrapper')
                );
            });
            ...
            targetDialogs = [
                ...
                bot.dialogSet.find(dialogToTest + ':hivekit-wrapper')
            ];
        }
