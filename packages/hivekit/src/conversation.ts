 ...
 */
import { Hivekit, HivekitMessage } from './core';
import { BotWorker } from './botworker';
import { HivekitDialogWrapper } from './dialogWrapper';
...
const debug = Debug('hivekit:conversation');

/**
 * Definition of the handler functions used to handle .ask and .addQuestion conditions
 */
interface HivekitConvoHandler {
    (answer: string, convo: HivekitDialogWrapper, bot: BotWorker, message: HivekitMessage): Promise<any>;
}

/**
 * Definition of the trigger pattern passed into .ask or .addQuestion
 */
interface HivekitConvoTrigger {
    type?: string;
    pattern?: string | RegExp;
    handler: HivekitConvoHandler;
    default?: boolean;
}

/**
 * Template for definiting a HivekitConversation template
 */
interface HivekitMessageTemplate {
    ...
    collect: {
        key?: string;
        options?: HivekitConvoTrigger[];
    };
}

export interface HivekitConversationStep {
    /**
 ...
 * ```
 */
export class HivekitConversation<O extends object = {}> extends Dialog<O> {
    /**
     ...
     * @param controller A pointer to the main Hivekit controller
     */
    public constructor(dialogId: string, controller: Hivekit) {
        super(dialogId);
        ...
        // TODO: maybe this ends up being managed by Hivekit
     ...
     */
    public say(message: Partial<HivekitMessageTemplate> | string): HivekitConversation {
     ...
     */
    public addAction(action: string, thread_name = 'default'): HivekitConversation {
     ...
     */
    public addChildDialog(dialog_id: string, key_name?: string, thread_name = 'default'): HivekitConversation {
        this.addQuestion({
            action: 'beginDialog',
            execute: {
                script: dialog_id
     ...
     * @param thread_name the name of a thread to which this call should be added. defaults to 'default'
     */
    public addGotoDialog(dialog_id: string, thread_name = 'default'): HivekitConversation {
        this.addMessage({
     ...
     */
    public addMessage(message: Partial<HivekitMessageTemplate> | string, thread_name: string): HivekitConversation {
        if (!thread_name) {
            thread_name = 'default';
        }
     ...
     */
    public ask(message: Partial<HivekitMessageTemplate> | string, handlers: HivekitConvoHandler | HivekitConvoTrigger[], key: {key: string} | string | null): HivekitConversation {
     ...
     * @param thread_name Name of thread to which message will be added
     */
    public addQuestion(message: Partial<HivekitMessageTemplate> | string, handlers: HivekitConvoHandler | HivekitConvoTrigger[], key: {key: string} | string | null, thread_name: string): HivekitConversation {
        if (!thread_name) {
            thread_name = 'default';
        }
     ...
     */
    public before(thread_name: string, handler: (convo: HivekitDialogWrapper, bot: BotWorker) => Promise<any>): void {
     ...
     */
    private async runBefore(thread_name: string, dc: DialogContext, step: HivekitConversationStep): Promise<void> {
        ...
        if (this._beforeHooks[thread_name]) {
            ...
            const convo = new HivekitDialogWrapper(dc, step);
     ...
     * @param step the current step object
     */
    private async runOnChange(variable: string, value: any, dc: DialogContext, step: HivekitConversationStep): Promise<void> {
    ...
    private async makeOutgoing(dc: DialogContext, line: any, vars: any): Promise<any> {
        ...
        /*******************************************************************************************************************/
        // Map some fields into the appropriate places for processing by Hivekit/ Bot Framework
        ...
        return new Promise((resolve, reject) => {
            // run the outgoing message through the Hivekit send middleware
     ...
     * @param step The current step object
     */
    private async gotoThreadAction(thread: string, dc: DialogContext, step: HivekitConversationStep): Promise<any> {
        step.thread = thread;
        step.index = 0;
    ...
    /**
     * Accepts a Hivekit script action, and performs that action
    ...
    private async handleAction(path, dc, step): Promise<any> {
        ...
        if (path.handler) {
            ...
            await path.handler.call(this, response, convo, bot, dc.context.turnState.get('hivekitMessage') || dc.context.activity);
        ...
        case 'wait':
            ...
            return { status: DialogTurnStatus.waiting };
        default:
            // the default behavior for unknown action in hivekit is to gotothread
