 ...
 */
import { HivekitMessage } from './core';
...
export class TeamsBotWorker extends BotWorker {
   ...
   */
  public async replyWithTaskInfo(message: HivekitMessage, taskInfo: any): Promise<any> {
    ...
    public async onTurn(context: TurnContext, next: () => Promise<any>): Promise<void> {
        if (context.activity.type === 'invoke') {
            if (!context.activity.name && context.activity.channelId === 'msteams') {
                context.activity.channelData.hivekitEventType = 'cardAction';
            } else {
                ...
                case 'task/fetch':
                case 'task/submit':
                    context.activity.channelData.hivekitEventType = context.activity.name;
                    break;
                }
            }
        }
