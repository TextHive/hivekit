...
import { BotWorker, HivekitMessage } from 'hivekit';

/**
 * This is a specialized version of [Hivekit's core BotWorker class](#BotWorker) that includes additional methods for interacting with Google Hangouts.
 * It includes all functionality from the base class, as well as the extension methods below.
 *
 * When using the HangoutsAdapter with Hivekit, all `bot` objects passed to handler functions will include these extensions.
 */
export class HangoutsBotWorker extends BotWorker {
     ...
     */
    public async updateMessage(update: Partial<HivekitMessage>): Promise<any> {
     ...
     */
    public async deleteMessage(update: Partial<HivekitMessage>): Promise<any> {
     ...
     */
    public async replyWithNew(src: any, resp: Partial<HivekitMessage>): Promise<any> {
     ...
     * @param resp A reply message containing text and/or cards
     */
    public async replyWithUpdate(src: any, resp: Partial<HivekitMessage>): Promise<any> {
        resp = this.ensureMessageFormat(resp);
        if (src.type === 'card_clicked') {
            this.httpBody({
                actionResponse: {
    ...
    public async replyInThread(src, resp): Promise<any> {
        ...
        src.incoming_message.conversation.threadKey = 'hivekit/' + Math.random() * 100000 + '/' + Math.random() * 100000;
    ...
    public async startConversationInThread(spaceName: string, userId: string, threadKey?: string): Promise<any> {
        return this.changeContext({
            conversation: {
                ...
                threadKey: threadKey || 'hivekit/' + Math.random() * 100000 + '/' + Math.random() * 100000
            },
