...
const debug = Debug('hivekit:hangouts');
...
export class HangoutsAdapter extends BotAdapter {
     ...
     * @ignore
     */
    public hivekit_worker = HangoutsBotWorker;
    ...
    public async processActivity(req, res, logic: (context: TurnContext) => Promise<void>): Promise<void> {
        ...
        if (this.options.token && this.options.token !== event.token) {
            ...
            if (event.type === 'ADDED_TO_SPACE') {
                ...
                activity.channelData.hivekitEventType = event.space.type === 'ROOM' ? 'bot_room_join' : 'bot_dm_join';
            }

            if (event.type === 'REMOVED_FROM_SPACE') {
                activity.type = ActivityTypes.Event;
                activity.channelData.hivekitEventType = event.space.type === 'ROOM' ? 'bot_room_leave' : 'bot_dm_leave';
            }

            if (event.type === 'CARD_CLICKED') {
                activity.type = ActivityTypes.Event;
                activity.channelData.hivekitEventType = event.type.toLowerCase();
            }
