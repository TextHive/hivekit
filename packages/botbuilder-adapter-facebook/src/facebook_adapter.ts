...
const debug = Debug('hivekit:facebook');
...
export class FacebookAdapter extends BotAdapter {
     ...
     * @ignore
     */
    public hivekit_worker = FacebookBotWorker;
     ...
     * @param hivekit
     */
    public async init(hivekit): Promise<any> {
        debug('Add GET webhook endpoint for verification at: ', hivekit.getConfig('webhook_uri'));
        if (hivekit.webserver) {
            hivekit.webserver.get(hivekit.getConfig('webhook_uri'), (req, res) => {
