...
const debug = Debug('hivekit:twilio');
...
export class TwilioAdapter extends BotAdapter {
    ...
    /**
     * A specialized BotWorker for Hivekit that exposes Twilio specific extension methods.
     * @ignore
     */
    public hivekit_worker = TwilioBotWorker;
    ...
    public async processActivity(req, res, logic: (context: TurnContext) => Promise<void>): Promise<void> {
        if (await this.verifySignature(req, res) === true) {
            ...
            if (event.NumMedia && parseInt(event.NumMedia) > 0) {
                // specify a different event type for Hivekit
                activity.channelData.hivekitEventType = 'picture_message';
            }
