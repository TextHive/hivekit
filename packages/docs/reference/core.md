# Hivekit Core Class Reference

[&larr; Hivekit Documentation](../core.md) [&larr; Class Index](index.md) 

This is a class reference for all the methods exposed by the [hivekit](https://github.com/texthive/hivekit/tree/master/packages/hivekit) package.

## Classes


* <a href="#Hivekit" aria-current="page">Hivekit</a>
* <a href="#HivekitBotFrameworkAdapter" aria-current="page">HivekitBotFrameworkAdapter</a>
* <a href="#TeamsBotWorker" aria-current="page">TeamsBotWorker</a>
* <a href="#BotWorker" aria-current="page">BotWorker</a>
* <a href="#HivekitConversation" aria-current="page">HivekitConversation</a>
* <a href="#HivekitDialogWrapper" aria-current="page">HivekitDialogWrapper</a>
* <a href="#HivekitTestClient" aria-current="page">HivekitTestClient</a>
* <a href="#TeamsInvokeMiddleware" aria-current="page">TeamsInvokeMiddleware</a>

## Interfaces

* <a href="#HivekitConfiguration" aria-current="page">HivekitConfiguration</a>
* <a href="#HivekitConversationStep" aria-current="page">HivekitConversationStep</a>
* <a href="#HivekitHandler" aria-current="page">HivekitHandler</a>
* <a href="#HivekitMessage" aria-current="page">HivekitMessage</a>
* <a href="#HivekitPlugin" aria-current="page">HivekitPlugin</a>

---

<a name="Hivekit"></a>
## Hivekit
Create a new instance of Hivekit to define the controller for a conversational app.
To connect Hivekit to a chat platform, pass in a fully configured `adapter`.
If one is not specified, Hivekit will expose an adapter for the Microsoft Bot Framework.
...
### Create a new Hivekit()
**Parameters**

| Argument | Type | Description
|--- |--- |---
| config | [HivekitConfiguration](#HivekitConfiguration) | Configuration for this instance of Hivekit<br/>

Create a new Hivekit instance and optionally specify a platform-specific adapter.
By default, Hivekit will create a [BotFrameworkAdapter](https://docs.microsoft.com/en-us/javascript/api/botbuilder/botframeworkadapter?view=botbuilder-ts-latest).
...
|--- |--- |---
| PATH | string | The path of the main Hivekit SDK, used to generate relative paths
...
| dialogSet | DialogSet | A BotBuilder DialogSet that serves as the top level dialog container for the Hivekit app
...
| version | string | The current version of Hivekit Core
...
## Hivekit Class Methods
<a name="addDep"></a>
### addDep()
(For use by Hivekit plugins only) - Add a dependency to Hivekit's bootup process that must be marked as completed using `completeDep()`.
Hivekit's `controller.ready()` function will not fire until all dependencies have been marked complete.
...
For example, a plugin that needs to do an asynchronous task before Hivekit proceeds might do:
```javascript
...
```javascript
// Create a dialog -- `HivekitConversation` is just one way to create a dialog
const my_dialog = new HivekitConversation('my_dialog', controller);
my_dialog.say('Hello');

// Add the dialog to the Hivekit controller
...
### addPluginExtension()
(Plugins only) Extend Hivekit's controller with new functionality and make it available globally via the controller object.
...
| handler| [HivekitHandler](#HivekitHandler) | a handler function in the form `async(bot, dialog_results) => {}`<br/>
...
<a name="handleTurn"></a>
### handleTurn()
Accepts the result of a BotBuilder adapter's `processActivity()` method and processes it into a Hivekit-style message and BotWorker instance
...
| handler| [HivekitHandler](#HivekitHandler) | a function that will be called should the pattern be matched<br/>
...
### loadModules()
Load all Hivekit feature modules located in a given folder.
...
| handler| [HivekitHandler](#HivekitHandler) | a handler function that will fire whenever one of the named events is received.<br/>
...
| Argument | Type | description
|--- |--- |---
| handler|  | A function to run when Hivekit is booted and ready to run.<br/>
...
Causes Hivekit to first emit a special `shutdown` event, process any bound handlers, and then finally terminate the webserver.
...
| message (optional)| [HivekitMessage](#HivekitMessage) | An incoming message or event<br/>
...
| Argument | Type | description
|--- |--- |---
| plugin_or_function|  | A plugin module in the form of function(hivekit) {...} that returns {name, middlewares, init} or an object in the same form.<br/>




<a name="HivekitBotFrameworkAdapter"></a>
## HivekitBotFrameworkAdapter
...
* Changes userAgent to reflect Hivekit instead of BotBuilder
...
Then import this and other classes into your code:
```javascript
const { HivekitBotFrameworkAdapter } = require('hivekit');
```
...
| Name | Type | Description
|--- |--- |---
| hivekit_worker | [TeamsBotWorker](#TeamsBotWorker) | 

## HivekitBotFrameworkAdapter Class Methods
...
## TeamsBotWorker
This is a specialized version of [Hivekit's core BotWorker class](core.md#BotWorker) that includes additional methods for interacting with Microsoft Teams.
...
Then import this and other classes into your code:
```javascript
const { TeamsBotWorker } = require('hivekit');
```
...
| Argument | Type | description
|--- |--- |---
| message| [HivekitMessage](#HivekitMessage) | 
...
Then import this and other classes into your code:
```javascript
const { BotWorker } = require('hivekit');
```
...
| Argument | Type | Description
|--- |--- |---
| controller | [Hivekit](#Hivekit) | A pointer to the main Hivekit controller
...
| Name | Type | Description
|--- |--- |---
| controller |  | Get a reference to the main Hivekit controller.
...
### ensureMessageFormat()
Take a crudely-formed Hivekit message with any sort of field (may just be a string, may be a partial message object)
...
await original_context.sendActivity('send directly using the adapter instead of Hivekit');
```
...
| Argument | Type | description
|--- |--- |---
| src| Partial&lt;HivekitMessage&gt; | An incoming message, usually passed in to a handler function
...
<a name="HivekitConversation"></a>
## HivekitConversation
An extension on the [BotBuilder Dialog Class](https://docs.microsoft.com/en-us/javascript/api/botbuilder-dialogs/dialog?view=botbuilder-ts-latest) that provides a Hivekit-friendly interface for
...
const convo = new HivekitConversation('foo', controller);
convo.say('Hello!');
...
### Create a new HivekitConversation()
**Parameters**

| Argument | Type | Description
|--- |--- |---
| dialogId | string | A unique identifier for this dialog, used to later trigger this dialog
| controller | [Hivekit](#Hivekit) | A pointer to the main Hivekit controller<br/>

Create a new HivekitConversation object
...
## HivekitConversation Class Methods
<a name="addAction"></a>
### addAction()
...
let profileDialog = new HivekitConversation('PROFILE_DIALOG', controller);
...
let onboard = new HivekitConversation('ONBOARDING', controller);
...
[Learn more about building conversations &rarr;](../conversations.md#build-a-conversation)
```javascript
let parent = new HivekitConversation('parent', controller);
let child = new HivekitConversation('child', controller);
...
[Learn more about building conversations &rarr;](../conversations.md#build-a-conversation)
```javascript
let conversation = new HivekitConversation('welcome', controller);
...
```javascript
let convo = new HivekitConversation(MY_CONVO, controller);
...
<a name="HivekitDialogWrapper"></a>
## HivekitDialogWrapper
This class is used to provide easy access to common actions taken on active HivekitConversation instances.
These objects are passed into handlers bound to HivekitConversations using .before .onChange and conditional handler functions passed to .ask and .addQuestion
...
Then import this and other classes into your code:
```javascript
const { HivekitDialogWrapper } = require('hivekit');
```
...
### Create a new HivekitDialogWrapper()
**Parameters**

| Argument | Type | Description
|--- |--- |---
| dc | DialogContext | 
| step | [HivekitConversationStep](#HivekitConversationStep) | 
...
## HivekitDialogWrapper Class Methods
<a name="gotoThread"></a>
### gotoThread()
...
Then import this and other classes into your code:
```javascript
const { HivekitTestClient } = require('hivekit');
```
...
### Create a new HivekitTestClient()
**Parameters**
...
| bot | [Hivekit](#Hivekit) | (Required) The Hivekit bot that has the skill to test.
...
Create a HivekitTestClient to test a dialog without having to create a full-fledged adapter.

```javascript
let client = new HivekitTestClient('test', bot, MY_DIALOG, MY_OPTIONS);
...
## HivekitTestClient Class Methods
...
When used, causes Hivekit to emit special events for teams "invokes"
Based on https://github.com/microsoft/botbuilder-js/blob/master/libraries/botbuilder/src/teamsActivityHandler.ts
This allows Hivekit bots to respond directly to task/fetch or task/submit events, as an example.
To use this, bind it to the adapter before creating the Hivekit controller:
```javascript
const Hivekit = new Hivekit({...});
hivekit.adapter.use(new TeamsInvokeMiddleware());
...
Then import this and other classes into your code:
```javascript
const { TeamsInvokeMiddleware } = require('hivekit');
```
...
<a name="HivekitConfiguration"></a>
## Interface HivekitConfiguration
Defines the options used when instantiating Hivekit to create the main app controller with `new Hivekit(options)`

**Fields**

| Name | Type | Description
|--- |--- |---
| adapter | any | A fully configured BotBuilder Adapter, such as `botbuilder-adapter-slack` or `botbuilder-adapter-web`<br/>The adapter is responsible for translating platform-specific messages into the format understood by Hivekit and BotBuilder.<br/>
...
| disable_webserver | boolean | Disable webserver. If true, Hivekit will not create a webserver or expose any webhook endpoints automatically. Defaults to false.<br/>For an example of how to use your own Express, [see this sample code](https://github.com/texthive/hivekit/blob/main/packages/testbot/custom_express.js).
...
<a name="HivekitConversationStep"></a>
## Interface HivekitConversationStep
...
<a name="HivekitHandler"></a>
## Interface HivekitHandler
A handler function passed into `hears()` or `on()` that receives a [BotWorker](#botworker) instance and a [HivekitMessage](#hivekitmessage).  Should be defined as an async function and/or return a Promise.
...
<a name="HivekitMessage"></a>
## Interface HivekitMessage
Defines the expected form of a message or event object being handled by Hivekit.
...
<a name="HivekitPlugin"></a>
## Interface HivekitPlugin
