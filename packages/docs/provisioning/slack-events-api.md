# Configure Hivekit and the Slack Events API

Building a bot with Hivekit and the Slack Events API gives you access to all of the best tools and options available to create a feature-rich bot for Slack.

In order to get everything set up, you will need to configure a new Slack App inside the [Slack Developer Portal](http://api.slack.com/apps), and at the same time, configure a Hivekit-powered bot. It only takes a few moments, but there are a bunch of steps, so follow these instructions carefully. 
...
## 2. Turn on your Hivekit app

You will need to [install Hivekit](../platforms/slack.md#get-started) and run it before your bot can be configured with Slack.

Once you've collected your `clientId` and `clientSecret` tokens from Slack, you can start your Hivekit app. You will need to have a bot service online and reachable over the web to complete the following steps and finish setting up your bot.

## 3. Configure OAuth

Hivekit and Slack use the OAuth authentication system to grant bots access to connect to, read from, and send messages to Slack teams.
...
Next you will need to add your Request URL, like this: `https://YOURURL/api/messages`. When you finish typing, Slack will verify that this endpoint is properly configured. You must be running your Hivekit application at the URL specified for this to work.
...
* [Hivekit Slack readme](../platforms/slack.md) 
