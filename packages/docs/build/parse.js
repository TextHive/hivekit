...
function buildPlugins(dest) {
    ...
    let toctemplate = Handlebars.compile(fs.readFileSync(__dirname + '/toc.hbs', 'utf8'));
    console.log(JSON.stringify(index, null, 2));
    fs.writeFileSync(dest, toctemplate({index: plugins, title: 'Hivekit Plugins'}));
}
...
function generateReference(src, dest) {
    ...
    classes = classes.sort(hivekitAtTop);
...
function hivekitAtTop(a,b) {
    if (a.name.match(/^hivekit$/i)) { 
        return -1;
    } else if(b.name.match(/^hivekit$/i)) {
        return 1;
    } else {
        return 0;
    }
...
generateReference(__dirname + '/hivekit.json',__dirname + '/../reference/core.md');
...
generatePlugin(__dirname + '/../../hivekit-plugin-cms/readme.md', {name: 'Hivekit CMS Plugin'} , __dirname + '/../plugins/cms.md');
