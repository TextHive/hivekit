...
const debug = Debug('hivekit:webex');
...
export class WebexAdapter extends BotAdapter {
    ...
    /**
     * Name used by Hivekit plugin loader
     * @ignore
     */
    public name = 'Webex Adapter';

    /**
     * Object containing one or more Hivekit middlewares to bind automatically.
     * @ignore
     */
    public middlewares;
     ...
     * @ignore
     */
    public hivekit_worker = WebexBotWorker;
    ...
    public constructor(config: WebexAdapterOptions) {
        ...
        // Hivekit Plugin additions
        this.middlewares = {
            spawn: [
                async (bot, next): Promise<void> => {
                    // make webex api directly available on a hivekit instance.
                    bot.api = this._api;
    ...
    public init(hivekit): void {
        ...
        hivekit.addDep('webex-identity');

        this.getIdentity().then(() => {
            hivekit.completeDep('webex-identity');
        }).catch((err) => {
            throw new Error(err);
        });
        ...
        hivekit.ready(() => {
            ...
            hivekit.adapter.registerWebhookSubscription(hivekit.getConfig('webhook_uri'));
        });
    }
    ...
    public registerWebhookSubscription(webhook_path): void {
        const webhook_name = this.options.webhook_name || 'Hivekit Firehose';
    ...
    public registerAdaptiveCardWebhookSubscription(webhook_path): void {
        const webhook_name = this.options.webhook_name || 'Hivekit AttachmentActions';
    ...
    public async processActivity(req, res, logic: (context: TurnContext) => Promise<void>): Promise<any> {
        ...
        if (payload.resource === 'messages' && payload.event === 'created') {
            ...
            if (activity.from.id === this.identity.id) {
                activity.channelData.hivekitEventType = 'self_message';
                activity.type = ActivityTypes.Event;
            } else {
                // change the event type of messages sent in 1:1s
                if (activity.channelData.roomType === 'direct') {
                    activity.channelData.hivekitEventType = 'direct_message';
                }
            }
            ...
            activity.channelData.hivekitEventType = 'attachmentActions';
            ...
            activity = {
                ...
                channelData: {
                    ...payload,
                    hivekitEventType: payload.resource + '.' + payload.event
                },
                type: ActivityTypes.Event
            };
