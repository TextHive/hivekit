...
WebexAdapter provides a translation layer for Hivekit and BotBuilder so that bot developers can connect to Webex Teams and have access to Webex's API.

### Hivekit Basics

When used in concert with Hivekit, developers need only pass the configured adapter to the Hivekit constructor, as seen below. Hivekit will automatically create and configure the webhook endpoints and other options necessary for communicating with Webex.
...
## Hivekit Extensions
In Hivekit handlers, the `bot` worker for Webex contains [all of the base methods](../docs/reference/core.md#BotWorker) as well as the following platform-specific extensions:
